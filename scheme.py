...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
	...
	if scheme_symbolp(first) and first in SPECIAL_FORMS:
		...
		check_procedure(procedure)	# Check if argument is a Scheme procedure
		operands = expr.second		
		# Proceed to evalute operands with procedure
		return procedure.eval_call(operands, env)
		# END PROBLEM 5
...
def eval_all(expressions, env):
	...
	# BEGIN PROBLEM 8
	if expressions is nil: # Empty list of expressions
		return None
	elif expressions.second is nil:
		# Return the value of the final sub-expression
		return scheme_eval(expressions.first, env, True)
	# Evaluate sub-expression
	scheme_eval(expressions.first, env)
	# Evaluate the other sub-expression(s)
...
class Frame:
	...
	def make_child_frame(self, formals, vals):
		...
		# BEGIN PROBLEM 11
		# Raise an error if number of formals does not match number of values
		if not (len(formals) == len(vals)):
			raise SchemeError("Number of argument values does \
			not match number of formal parameters.")			
		# Bind every formal to its corresponding value
...
class Procedure:
	...
	def eval_call(self, operands, env):
		...
		# BEGIN PROBLEM 5
		# Create a lambda function that calls scheme_eval 
		# with env as the second arg.
		scheme_eval_lambda = lambda x: scheme_eval(x, env)		
		# Evaluate the operands
		evaluated_operands = operands.map(scheme_eval_lambda)		
...
class PrimitiveProcedure(Procedure):
	...
	def apply(self, args, env):
		...
		while args is not nil:
			...
			args = args.second
		# BEGIN PROBLEM 4
		# Check to see if this primitive procecure will expect the current
		# enviroment to be passed in as the last argument
		if self.use_env:
...
class LambdaProcedure(UserDefinedProcedure):
	...
	def make_call_frame(self, args, env):
		...
		child_frame = parent_frame.make_child_frame(self.formals, args)
		return child_frame
		# END PROBLEM 12
...
def do_define_form(expressions, env):
	...
	if scheme_symbolp(target):
		...
		# BEGIN PROBLEM 10
		# Create a new Pair of parameters and the body/bodies of
		# a lambda function
		new_pair_vals = Pair(target.second, expressions.second)
		env.define(target.first, do_lambda_form(new_pair_vals, env))
		return target.first
		# END PROBLEM 10
	else:
...
def do_lambda_form(expressions, env):
	...
	# BEGIN PROBLEM 9
	body = expressions.second #List of body expressions
...
def do_and_form(expressions, env):
	...
	if expressions is nil:
		...
		expression_value = scheme_eval(expressions.first, env, True)
		if scheme_truep(expression_value):
			return expression_value
		return False
	else:
		# Get the value of the sub-expression
		expression_value = scheme_eval(expressions.first, env)
		if scheme_truep(expression_value):
...
def make_let_frame(bindings, env):
	...
	# BEGIN PROBLEM 15
	names, expression_values = nil, nil	
	while bindings is not nil:
		...
		check_formals(names)
		expression_values = \
		Pair(scheme_eval(binding.second.first, env), expression_values)
		bindings = bindings.second
	child_frame = env.make_child_frame(names, expression_values)		
	return child_frame
	# END PROBLEM 15
